...
export type InputMaskFn = (value: string, selection: Selection) => [value: string, selection: Selection];

export type InputMaskArray = (string | RegExp)[];

export type ReplaceArgs = [substring: string, ...args: never[]];
...
export const maskPresets = {
	...
	oneDate: "99.99.9999",
	twoDates: "99.99.9999-99.99.9999",
};
		...
		[
			value.replace(regex, (...args) => {
				...
				selection[0] +=
					index < selection[0]
						? 0
						: ((replacement.length - args[0].length) / args[0].length) * Math.max(selection[0] - index, args[0].length);
				selection[1] +=
					index < selection[1]
						? 0
						: ((replacement.length - args[0].length) / args[0].length) * Math.max(selection[1] - index, args[0].length);
				return replacement;
			}),
			selection,
		];
...
export const anyMaskToFn = (mask: InputMask, regexps?: Record<string, RegExp>) =>
	typeof mask === "function"
		...
		: maskArrayToFn(Array.isArray(mask) ? (mask as InputMaskArray) : stringMaskToArray(mask, regexps));
 ...
 */
export const createInputMask = <MaskEvent extends EventLike = KeyboardEvent | InputEvent | ClipboardEvent | FormEvent>(
	mask: InputMask,
...
export const formatByMask = (value: string, mask: InputMask, regexps?: Record<string, RegExp>) => {
	const maskFn = anyMaskToFn(mask, regexps);
	const [formattedValue] = maskFn(value, [value.length, value.length]);
